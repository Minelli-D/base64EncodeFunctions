...
const toBase64 = function (str) {
    ...
    let binaryListOneTake = addEndZeros(textToBinary(str));
    let sliceToSixGroup = chunkArray(binaryListOneTake.join('').split(""), 6);
    //traslate the decimal(bit value) checking the base64Map
    let codedString = [];
    sliceToSixGroup.forEach((e) => {
        let t = parseInt(e.join(''), 2);
        codedString.push(base64Map.get(t));
    });
    //Add padding, check the latest 2Byte
    let arrayForPadding = chunkArray(codedString, 4);
